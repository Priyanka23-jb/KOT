...
export default function Sidebar() {
  const role = useSelector((state) => state.auth.user?.role); // admin | vendor
  ...
  return (
    <aside className="w-64 bg-white dark:bg-gray-800 text-gray-800 dark:text-gray-200 h-full border-r">
      ...
      <nav className="p-4 space-y-1">
        ...
        <div>
          <button onClick={() => toggleSection("customer")} className={sectionBtn}>
            <span className="font-medium">Customer</span>
            <ChevronDown size={16} />
          </button>
          ...
          {openSections.customer && (
            <div className="ml-4 border-l pl-3">
              <SidebarItem to="/customer/solar-registration" label="Customer Registration" indent />
              <SidebarItem to="/customer/solar-list" label="Customer List" indent />
            </div>
          )}
        </div>
        ...
        {/* ================= VENDOR (ADMIN + VENDOR) ================= */}
        <div>
          <button onClick={() => toggleSection("vendor")} className={sectionBtn}>
            <span className="font-medium">Vendor</span>
            <ChevronDown size={16} />
          </button>
        ...
        {/* ================= TECHNICIAN (ADMIN + VENDOR) ================= */}
        <div>
          <button onClick={() => toggleSection("technician")} className={sectionBtn}>
            <span className="font-medium">Technician</span>
            <ChevronDown size={16} />
          </button>
          ...
          {openSections.technician && (
            <div className="ml-4 border-l pl-3">
              <SidebarItem to="/technician/create" label="Create Technician" indent />
              <SidebarItem to="/technician/list" label="Technician List" indent />
            </div>
          )}
        </div>
        ...
        {role === "admin" && (
          <>
            {/* CONTAIN */}
            <div>
              <button onClick={() => toggleSection("contain")} className={sectionBtn}>
                <span className="font-medium">Contain Management</span>
                <ChevronDown size={16} />
              </button>
            ...
            {/* AUDITOR */}
            <div>
              <button onClick={() => toggleSection("auditor")} className={sectionBtn}>
                <span className="font-medium">Auditor</span>
                <ChevronDown size={16} />
              </button>
            ...
            {/* SUBADMIN */}
            <div>
              <button onClick={() => toggleSection("subadmin")} className={sectionBtn}>
                <span className="font-medium">Subadmin</span>
                <ChevronDown size={16} />
              </button>
            ...
            {/* MASTER */}
            <div>
              <button onClick={() => toggleSection("master")} className={sectionBtn}>
                <span className="font-medium">Master</span>
                <ChevronDown size={16} />
              </button>
              ...
              {openSections.master && (
                <div className="ml-4 border-l pl-3">
                  <SidebarItem to="/location" label="Location" indent />
                  <SidebarItem to="/category" label="Category" indent />
                  <SidebarItem to="/department" label="Department" indent />
                  <SidebarItem to="/designation" label="Designation" indent />
                </div>
              )}
            </div>
          </>
        ...
        {/* HELP (OPTIONAL – KEEP IF YOU WANT) */}
        <SidebarItem to="/help" label="Help" />
      </nav>
    </aside>
  );
